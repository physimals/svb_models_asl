...
class AslRestDisp(AslRestModel):
    ...
    OPTIONS = AslRestModel.OPTIONS + [
        ModelOption(
            "conv_dt",
            "Time interval for numerical convolution",
            units="s",
            type=float,
            default=0.1,
        ),
        ModelOption(
            "conv_type",
            "Convolution type ('gamma' only supprted type presently)",
            type=str,
            default="gamma",
        ),
        ModelOption(
            "infer_disp_params",
            "Whether to infer parameters of the dispersion",
            type=bool,
            default=True,
        ),
    ]
    ...
    def tissue_signal(self, t, ftiss, delt, t1, extra_params):
        ...
        aif = self.log_tf(
            self.aif_gammadisp(conv_t, delt, extra_params),
            force=False,
            shape=True,
            name="aif",
        )  # [W, S, NT]
        resid = self.log_tf(
            self.resid_wellmix(conv_t, t1), force=False, shape=True, name="resid"
        )  # [NT]
        kinetic_curve = self.log_tf(
            self.conv_tf(aif, resid, self.conv_dt), force=False, shape=True, name="conv"
        )  # [W, S, NT]

        # Sample the kinetic curve at the TIs (using linear interpolation here)
        signal = self.log_tf(
            tfp.math.batch_interp_regular_1d_grid(
                t, 0, self.conv_tmax, kinetic_curve, axis=-1
            ),
            force=False,
            shape=True,
            name="sig",
    ...
    def aif_gammadisp(self, t, delt, extra_params):
        ...
        post_bolus = self.log_tf(
            tf.greater(t, tf.add(delt, self.tau), name="aif_pre_bolus"), shape=True
        )
        during_bolus = tf.logical_and(
            tf.logical_not(pre_bolus), tf.logical_not(post_bolus)
        )

        # Calculate AIF
        if self.casl:
            kcblood_nondisp = self.log_tf(
                2 * tf.exp(-delt / self.t1b),
                name="kcblood_nodisp",
                force=False,
                shape=True,
            )
        ...
        k = 1 + sp
        gamma1 = self.log_tf(
            tf.math.igammac(k, s * tf.clip_by_value(t - delt, 0, 1e6)),
            name="gamma1",
            force=False,
            shape=True,
        )
        gamma2 = self.log_tf(
            tf.math.igammac(k, s * tf.clip_by_value(t - delt - self.tau, 0, 1e6)),
            name="gamma2",
            force=False,
        )
    ...
    def aif_nodisp(self, t, delt, extra_params):
        ...
        pre_bolus = self.log_tf(tf.less(t, delt, name="aif_pre_bolus"), shape=True)
        post_bolus = self.log_tf(
            tf.greater(t, tf.add(delt, self.tau), name="aif_post_bolus"), shape=True
        )
        during_bolus = tf.logical_and(
            tf.logical_not(pre_bolus), tf.logical_not(post_bolus)
        )
        ...
        if self.casl:
            kcblood_nondisp = tf.repeat(
                2 * tf.exp(-delt / self.t1b), tf.shape(t)[0], axis=-1
            )
        else:
            kcblood_nondisp = 2 * tf.exp(-t / self.t1b)
    ...
    def conv_tf(self, data, kernel, dt):
        ...
        data = tf.expand_dims(
            tf.reshape(data, [-1, data_shape[-1]]), -1
        )  # [W, NT, 1] or [W*S, NT, 1]
